//GENERATE_START
<!-- The Aiouniya Business Entity manangement Component -->
<template>
...
<script>
	...
	import AiouniyaEditor from '@/views/aiouniya/editor'
  	export default {
		// 注册依赖组件
		components : {AiouniyaEditor},
    	data() {
      	  	return {
				// 定义表格字段
        		header: [
    				{title: this.$t('aiouniya.qmdj'), field: 'qmdj', class: ''},   
    				{title: this.$t('aiouniya.abab'), field: 'abab', class: ''},   
    				{title: this.$t('aiouniya.aabb'), field: 'aabb', class: ''},   
        		],
				// 翻页数据格式
				pagination : {
					page: 1,
    	...
    	methods: {
			doSearch() {// 执行查询
				getJson('api/business/aiouniya/paginate', this.conditions, response => {
			...
			saveEntity(){// 保存和修改功能
				...
				if(mode == 'NEW'){
					postJson('api/business/aiouniya', entity, response =>{
						this.$refs.editor.close()
						this.doSearch()
					})
				}else{
					putJson('api/business/aiouniya', entity, response =>{
						this.$refs.editor.close()
						this.doSearch()
					})
				}
			...
			deleteEntity(item){// 删除功能
	            this.$confirm(this.$t('aiouniya.delete_confirm', [item.null]), this.$t('confirm'), {
	              	confirmButtonText: this.$t('actions.ok'),
	              	cancelButtonText: this.$t('actions.cancel'),
	              	type: 'warning'
	            }).then(() => {
	              	deleteJson('api/business/aiouniya/' + item.null, {}, response => {
	              		this.$message({type: 'info', message: this.$t('aiouniya.delete_success', [item.null])})
						this.doSearch()
	              	})
	            }).catch(() => {
	              	this.$message({
