...
<template>
	<el-dialog :title="title" :visible.sync="visible">
		<el-form v-if="visible == true" ref="saveForm" :model="entity" :rules="rules" status-icon label-width="120px">
			<el-form-item :label="$t('gailun.qmdj')">
			   <el-input v-if="!''" v-model.trim="entity.qmdj"></el-input>
			</el-form-item>
			<el-form-item :label="$t('gailun.abab')">
			   <el-input v-if="!''" v-model.trim="entity.abab"></el-input>
			</el-form-item>
			<el-form-item :label="$t('gailun.aabb')">
	...
	export default {
		...
		methods:{
			open(entity){
				...
				if(this.entity.null){
					this.mode = 'EDIT'
					this.title = this.$t('gailun.edit')
				}else{
					this.mode = 'NEW'
					this.title = this.$t('gailun.addnew')
				}
			},
			getMode(){
				return this.mode
